...
//AGREGAR COLUMNA 'ACCION'
		foreach ($ArrayDeProductos as $prod){
			$codigoBarra = $prod->GetCodBarra();
		
			$grilla .= "<tr>
							<td>".$codigoBarra."</td>
							<td>".$prod->GetNombre()."</td>
							<td><img src='archivos/".$prod->GetPathFoto()."' width='100px' height='100px'/></td>
							<td style=display:block><input type='button' value='Modificar' onclick=></td>
							<td style=display:block><input type='button' value='Eliminar' onclick=Main.EliminarProducto($codigoBarra)></td>
						</tr>						
						";
