...
//--METODOS DE CLASE
	...
	public static function Modificar($obj)
	{
		...
		$listaDeProductos = Producto::TraerTodosLosProductos();
	...
	public static function Eliminar($codBarra)
	{
		...
		foreach($listaDeProductos as $item){
			if($item->codBarra == $codBarra){
				$pathImg = $item->pathFoto;
				$producto = $item;												
				break;
			}
			$contador++;				
		}
		unset($listaDeProductos[$contador]);

		
		//BORRO LA IMAGEN ANTERIOR
		$pathImg = trim($pathImg);
		echo $pathImg;
		$direccion = "../archivos/".$pathImg;
		
		if(file_exists("../archivos/".$pathImg))
		echo "existe";
		else
		echo "no existe";
		die();

		unlink("../../archivos/".$pathImg);		
		//ABRO EL ARCHIVO
		$ar = fopen("archivos/productos.txt", "a");
		//ESCRIBO EN EL ARCHIVO
		foreach($listaDeProductos as $item){
